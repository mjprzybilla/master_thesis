#####################################################################################################################################
##                                                                                                                      
##  Perform clustering and umap embedding on 10x single-cell RNA-sequencing data from Early and Late organoids and Sathe et al. UMAP
##
##  Date: 16 May 2020                                                                                                                   
##  
...
## SATHE DATASET
# set sample name
sample.tmp <- "EL_Seq13_19_27_Sathe"
...
seurat.obj.big <- readRDS(paste0("/labs/ccurtis2/mjprzy/scRNA_analysis/LSI_projection/merged_data_SatheEtal_freeze/merged_data_SatheEtal_freeze_iterativeLSI_seurat.obj.rds"))
...
se <- readRDS("/labs/ccurtis2/mjprzy/scRNA_analysis/LSI_projection/merged_data_SatheEtal_freeze/SummarizedExperiment_merged_data_SatheEtal_freeze.rds")

#Load Saved UMAP Manifold
umapManifold <- uwot::load_uwot("/labs/ccurtis2/mjprzy/scRNA_analysis/LSI_projection/merged_data_SatheEtal_freeze/SummarizedExperiment_merged_data_SatheEtal_freeze_UMAP-model.uwot")
...
# SATHE DATASET
refDF$sample_type <- paste0(str_split_fixed(rownames(refDF), "_", 3)[,1], "_", str_split_fixed(rownames(refDF), "_", 3)[,2])
refDF$sample_type [grep("n1", refDF$sample_type )] <- "Normal"
refDF$sample_type [grep("n2", refDF$sample_type )] <- "Normal"
refDF$sample_type [grep("t1", refDF$sample_type )] <- "Tumor"
refDF$sample_type [grep("t2", refDF$sample_type )] <- "Tumor"
...
######################################################################################
# PLOT UMAP OF PROJECTION WITH COLORING ACCORDING TO SAMPLE TYPES IN SATHE DATASET
######################################################################################
...
ggplot(projectionDF, aes(X1,X2,color=sample_type)) + 
  ...
  xlab("UMAP Dimension 1") + 
  ylab("UMAP Dimension 2") +
  scale_color_manual(values=c("Normal"="lightgrey", "Tumor"="aliceblue", "6077" = "chartreuse4", "4230"="dodgerblue3","0891"="firebrick3")) +
  theme_classic() +
  theme(axis.title = element_text(size = 40, face = "bold")) +
  theme(legend.position="bottom", legend.title=element_text(size=25, face = "bold")) + 
  theme(legend.text = element_text(colour="black", size=25, face="bold"),
...
######################################################################################
# PLOT PROJECTION ACCORDING TO INDIVIDUAL PATIENTS - SATHE
######################################################################################
...
ggplot(P1.projectionDF, aes(X1,X2,color=sample_type)) +
  ...
  xlab("UMAP Dimension 1") +
  ylab("UMAP Dimension 2") +
  scale_color_manual(values=c("background" = "lightgrey","Normal"="lightgrey", "Tumor"="lightgrey", "0891" = "chartreuse4")) +
  theme_classic() +
  theme(axis.title = element_text(size = 40, face = "bold")) +
  theme(legend.position="bottom", legend.title=element_text(size=25, face = "bold")) +
  theme(legend.text = element_text(colour="black", size=25, face="bold"),
...
ggplot(P2.projectionDF, aes(X1,X2,color=sample_type)) +
  ...
  xlab("UMAP Dimension 1") +
  ylab("UMAP Dimension 2") +
  scale_color_manual(values=c("Normal"="lightgrey", "Tumor"="lightgrey", "6077" = "dodgerblue3")) +
  theme_classic() +
  theme(axis.title = element_text(size = 40, face = "bold")) +
  theme(legend.position="bottom", legend.title=element_text(size=25, face = "bold")) +
  theme(legend.text = element_text(colour="black", size=25, face="bold"),
...
ggplot(P3.projectionDF, aes(X1,X2,color=sample_type)) +
  ...
  xlab("UMAP Dimension 1") +
  ylab("UMAP Dimension 2") +
  scale_color_manual(values=c("Normal"="lightgrey", "Tumor"="lightgrey", "4230" = "firebrick3")) +
  theme_classic() +
  theme(axis.title = element_text(size = 40, face = "bold")) +
  theme(legend.position="bottom", legend.title=element_text(size=25, face = "bold")) +
  theme(legend.text = element_text(colour="black", size=25, face="bold"),
...
# change 6077 WT
freq.table <- rbind(freq.table, freq.table[5,])
rownames(freq.table)[5] <- "6077_C5_WT"
rownames(freq.table)[18] <- "6077_D3_WT"

# change 0891 WT
freq.table <- rbind(freq.table, freq.table[17,])
rownames(freq.table)[17] <- "0891_P31_WT"
rownames(freq.table)[19] <- "0891_P41_WT"

# change 4230 WT
freq.table <- rbind(freq.table, freq.table[12,],freq.table[12,])
rownames(freq.table)[12] <- "4230_APA6_WT"
rownames(freq.table)[20] <- "4230_APA4_WT"
rownames(freq.table)[21] <- "4230_PB6_WT"
...
# replace the clone ids with the patient ids
## 0891
melt.freq.table[grep("0891_P31", melt.freq.table$sample_id),"clone_id"] <- "P1C2"
melt.freq.table[grep("0891_P41", melt.freq.table$sample_id),"clone_id"] <- "P1C3"
melt.freq.table[grep("0891_P31_WT", melt.freq.table$sample_id),"clone_id"] <- "P1C2"
melt.freq.table[grep("0891_P41_WT", melt.freq.table$sample_id),"clone_id"] <- "P1C3"
## 6077
melt.freq.table[grep("6077_C5", melt.freq.table$clone_id),"clone_id"] <- "P2C2"
melt.freq.table[grep("6077_D3", melt.freq.table$clone_id),"clone_id"] <- "P2C3"
melt.freq.table[grep("6077_C5_WT", melt.freq.table$sample_id),"clone_id"] <- "P2C2"
melt.freq.table[grep("6077_D3_WT", melt.freq.table$sample_id),"clone_id"] <- "P2C3"
## 4230
melt.freq.table[grep("4230_PB6", melt.freq.table$clone_id),"clone_id"] <- "P3C1"
melt.freq.table[grep("4230_APA4", melt.freq.table$clone_id),"clone_id"] <- "P3C2"
melt.freq.table[grep("4230_APA6", melt.freq.table$clone_id),"clone_id"] <- "P3C3"
melt.freq.table[grep("4230_PB6_WT", melt.freq.table$sample_id),"clone_id"] <- "P3C1"
melt.freq.table[grep("4230_APA4_WT", melt.freq.table$sample_id),"clone_id"] <- "P3C2"
melt.freq.table[grep("4230_APA6_WT", melt.freq.table$sample_id),"clone_id"] <- "P3C3"
