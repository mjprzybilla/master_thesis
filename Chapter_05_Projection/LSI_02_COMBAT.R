...
Zhang.epithelial.seurat.obj <- readRDS("/labs/ccurtis2/mjprzy/scRNA_analysis/hashECB_data_freeze/merged_data_ZhangEtal/merged_data_ZhangEtal_Epithelial_CellType_seurat_obj.rds")
Zhang.matrix <- Zhang.epithelial.seurat.obj@assays$SCT@counts

# show dimensions
print(dim(Zhang.matrix))

# get input seurat objects from organoids
EL.organoid.seurat.obj <- readRDS("/labs/ccurtis2/mjprzy/scRNA_analysis/hashECB_data_freeze/EL_Seq13_19_27/EL_Seq13_19_27_CellType_seurat_obj.rds")
EL.matrix <- EL.organoid.seurat.obj@assays$SCT@counts

# show dimensions
print(dim(EL.matrix))

# make a metadata sheet including cell_id outcome batch and wether its organoid or tissue with cell barcodes as rownames
EL.organoid.metadata <- data.frame(sample_id = EL.organoid.seurat.obj@meta.data[,2])
rownames(EL.organoid.metadata) <- rownames(EL.organoid.seurat.obj@meta.data)
EL.organoid.metadata$cell <- c(1:nrow(EL.organoid.metadata))
EL.organoid.metadata$batch <- 1
EL.organoid.metadata$type <- "Organoid"

# for the Zhang dataset
Zhang.epithelial.metadata <- data.frame(sample_id = Zhang.epithelial.seurat.obj@meta.data[,1])
rownames(Zhang.epithelial.metadata) <- rownames(Zhang.epithelial.seurat.obj@meta.data)
Zhang.epithelial.metadata$cell <- c((nrow(EL.organoid.metadata)+1):(nrow(EL.organoid.metadata)+nrow(Zhang.epithelial.metadata)))
Zhang.epithelial.metadata$batch <- 0
Zhang.epithelial.metadata$type <- "Tissue"

# bind info together
s.info <- rbind(EL.organoid.metadata, Zhang.epithelial.metadata)
...
matrix.list <- list(EL.matrix, Zhang.matrix)
...
EL.combat.bc.data <- combat.bc.data[, colnames(combat.bc.data) %in% colnames(EL.matrix)]
