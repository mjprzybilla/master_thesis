################################################################################################################################################
##                                                                                                                      
##  Plot subset of gene expression matrix based on the source code from honeybadger (credit to Jean Fan) for Sequencing 25                                                                
##                                                                                                                      
##  Date: 11 April 2020                                                                                                                    
##  
...
Seq25.norm.gExp.matrices <- list.files("/labs/ccurtis2/mjprzy/infercnv_gastric/freeze/Sequencing25_6077_C5_T2_T12_WT", pattern = "infercnv.median_filtered.observations.txt", recursive = T, full.names = T)
Seq25.annotation.files <- list.files("/labs/ccurtis2/mjprzy/infercnv_gastric/freeze/Sequencing25_6077_C5_T2_T12_WT", pattern = "cellAnnotations", recursive = T, full.names = T)
Seq25.gene.ordering.files <- list.files("/labs/ccurtis2/mjprzy/infercnv_gastric/freeze/Sequencing25_6077_C5_T2_T12_WT", pattern = "gene_ordering_file.txt", recursive = T, full.names = T)

# read in metadata from seurat object
seurat.obj <- readRDS("/labs/ccurtis2/mjprzy/scRNA_analysis/hashECB_data_freeze/Sequencing25_6077_C5_T2_T12/Sequencing25_6077_C5_T2_T12_seurat_obj.rds")
metadata <- seurat.obj@meta.data
metadata <- metadata[,c(1,5,6,7)]

i <- 5
for (i in 1:length(Seq25.norm.gExp.matrices)){
  
  # get sample id
  sample.tmp <- str_split_fixed(Seq25.norm.gExp.matrices[i], "/", 9)[,8]
  print(sample.tmp)
  
  # read in normalized matrix
  norm.gExp.matrix <- as.matrix(read.table(Seq25.norm.gExp.matrices[i], header = T, sep = " "))
  colnames(norm.gExp.matrix) <- str_split_fixed(colnames(norm.gExp.matrix), "\\.", 2)[,1]
  print(dim(norm.gExp.matrix))
  
  # read in annotation file
  annotation.file <- read.table(Seq25.annotation.files[i], header = F, sep = "\t", stringsAsFactors = F)
  ...
  gene.ordering.file <- read.table(Seq25.gene.ordering.files[i], header = F, col.names = c("hgnc_symbol", "chr", "start", "end"), sep = "\t")
  ...
  # ECB clones
  ECB.clones <- table(sample.metadata$ECB_RG) > 10
  ...
  ECB.hc.metadata <- merge(sample.metadata, unique.ECB_RG, by="ECB_RG")
  ...
  png(paste0("/labs/ccurtis2/mjprzy/infercnv_gastric/freeze/Sequencing25_6077_C5_T2_T12_WT/", sample.tmp, "/", sample.tmp, "_w_ECB.png") , width = 13, height = 4, units = 'in', res = 600)
  ...
  png(paste0("/labs/ccurtis2/mjprzy/infercnv_gastric/freeze/Sequencing25_6077_C5_T2_T12_WT/", sample.tmp, "/", sample.tmp, "_w_ECB_legend.png"), width = 13, height = 4, units = 'in', res = 600, type = "cairo")
