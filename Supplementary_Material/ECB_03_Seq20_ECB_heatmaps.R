...
list.of.packages <- c("biomaRt", "Matrix", "stringr", "GenomicRanges", "scater", "fields")
...
Seq20.norm.gExp.matrices <- list.files("/labs/ccurtis2/mjprzy/infercnv_gastric/freeze/Sequencing20_C1_T2_T7_T12_ECB_WT", pattern = "infercnv.median_filtered.observations.txt", recursive = T, full.names = T)
Seq20.annotation.files <- list.files("/labs/ccurtis2/mjprzy/infercnv_gastric/freeze/Sequencing20_C1_T2_T7_T12_ECB_WT", pattern = "cellAnnotations", recursive = T, full.names = T)
Seq20.gene.ordering.files <- list.files("/labs/ccurtis2/mjprzy/infercnv_gastric/freeze/Sequencing20_C1_T2_T7_T12_ECB_WT", pattern = "gene_ordering_file.txt", recursive = T, full.names = T)

# read in metadata from seurat object
seurat.obj <- readRDS("/labs/ccurtis2/mjprzy/scRNA_analysis/hashECB_data_freeze/Sequencing20_C1_T2_T7_T12_ECB/Sequencing20_C1_T2_T7_T12_ECB_seurat_obj.rds")
metadata <- seurat.obj@meta.data
metadata <- metadata[,c(1,5,6,7)]

i <- 7
for (i in 1:length(Seq20.norm.gExp.matrices)){
  
  # get sample id
  sample.tmp <- str_split_fixed(Seq20.norm.gExp.matrices[i], "/", 9)[,8]
  print(sample.tmp)
  
  # read in normalized matrix
  norm.gExp.matrix <- as.matrix(read.table(Seq20.norm.gExp.matrices[i], header = T, sep = " "))
  colnames(norm.gExp.matrix) <- str_split_fixed(colnames(norm.gExp.matrix), "\\.", 2)[,1]
  
  # read in annotation file
  annotation.file <- read.table(Seq20.annotation.files[i], header = F, sep = "\t", stringsAsFactors = F)
  ...
  gene.ordering.file <- read.table(Seq20.gene.ordering.files[i], header = F, col.names = c("hgnc_symbol", "chr", "start", "end"), sep = "\t")
  ...
  png(paste0("/labs/ccurtis2/mjprzy/infercnv_gastric/freeze/Sequencing20_C1_T2_T7_T12_ECB_WT/", sample.tmp, "/", sample.tmp, "_w_ECB.png") , width = 26, height = 12, units = 'in', res = 600)
  ...
  png(paste0("/labs/ccurtis2/mjprzy/infercnv_gastric/freeze/Sequencing20_C1_T2_T7_T12_ECB_WT/", sample.tmp, "/", sample.tmp, "_w_ECB_legend.png"), width = 13, height = 4, units = 'in', res = 600, type = "cairo")
