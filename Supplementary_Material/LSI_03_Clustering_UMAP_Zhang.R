...
## ZHANG DATASET
# set sample name
sample.tmp <- "merged_data_ZhangEtal_freeze"
dir.create(sample.tmp)

# read in the seurat object for the annotated Sathe dataset
# this only comprises epithelial cells
seurat.obj.big <- readRDS(paste0(w.dir, "merged_data_ZhangEtal/merged_data_ZhangEtal_Epithelial_CellType_seurat_obj.rds"))
reference.data <- seurat.obj.big@assays$RNA@counts

# HERE I REMOVE THE CELLS WHICH TURNED OUT TO BE NON-EPITHELIAL CELLS IN THE CLUSTERING
# remove cells which are not epithelial cells
remove.cells <- read.table("/labs/ccurtis2/mjprzy/scRNA_analysis/LSI_projection/merged_data_ZhangEtal_freeze/merged_data_ZhangEtal_freeze_removeCells.txt", col.names = "Cell_barcodes", sep = "\t")
...
seurat.obj.big <- SubsetData(seurat.obj.big, cells = valid.cells)
...
# dim: 15618 21968 
# metadata(23): orig.ident nCount_RNA ... Celltype SCT_snn_res.0.6
# assays(1): counts
# rownames(15618): STPG1 NIPAL3 ... TBX1 TDRD1
# rowData names(1): hgnc_symbol
# colnames(21968): CAG1_AAACCTGCACAGGTTT-1 CAG1_AAACCTGCATCCTTGC-1 ... NAG3_TTTGTCAAGCGTTCCG-1 NAG3_TTTGTCAGTCGACTGC-1
...
# remove.barcodes <- data.frame(Cell_barcodes = rownames(seurat.obj.big@meta.data[seurat.obj.big$iterativeLSI_clusters == 13, ]))
# write.table(remove.barcodes, "/labs/ccurtis2/mjprzy/scRNA_analysis/LSI_projection/merged_data_ZhangEtal_freeze/merged_data_ZhangEtal_freeze_removeCells.txt", sep = "\t", quote = F, col.names = T)
...
new.cluster.ids <- c("PMCs", "GMCs", "Endocrine Cells", "Chief Cells", "PMCs", "Enteroendocrine Cells", "Enteroendocrine Cells", "PMCs",
                     "Neck-like Cells", "MSCs", "PMCs", "Neck-like Cells", "Enteroendocrine Cells", "PMCs", "Neck-like Cells", "PMCs", "PMCs",
                     "GMCs", "Goblet Cells","Malignant Cells", "MSCs", "Enterocytes", "MSCs", "Malignant Cells", "Enterocytes", "MSCs")
