#############################################################################################################################
##                                                                                                                      
##  Analysis of individual subclones in scRNA-seq with ECB information from Sequencing 20
##                                                                                                                      
##  Date: 23 July 2020                                                                                                                    
##  
...
getAveAltUp <- function(rg,propup){
  ...
  degs <- read.table(paste0(sample.tmp, "/DEG/markers_",rg,"_vs_2.txt"), sep="\t", header = T, stringsAsFactors = T)
...
plotDEGs <- function(rg){
  ...
  pdf(paste0(o.dir, "/", sample.tmp, "/DEG/markers_",rg,"_vs_2.pdf"),width=9.5,height=1.5,pointsize=0.1)
...
Idents(seurat.obj) <- "SCT_snn_res.0.6"

# subset to only one timepoint
seurat.obj <- subset(seurat.obj, HashTag == "6077_P_C1_ECB_R2_T7")
...
cnv.clone.files <- cnv.clone.files[grep("6077_P_C1_ECB_R2_T7", cnv.clone.files)]
...
seurat.metadata <- seurat.metadata[seurat.metadata$HashTag == "6077_P_C1_ECB_R2_T7",]

# subset to barcode 0
clone0.barcodes <- seurat.metadata[seurat.metadata$ECB_RG == "0", "Cell_Barcode"]
clone1.barcodes <- seurat.metadata[seurat.metadata$ECB_RG == "1", "Cell_Barcode"]

# subset the clone file to subclone 0
clone0.file <- clone.file[clone.file$Cell_barcodes %in% clone0.barcodes, ]
clone0a.barcodes <- clone0.file[clone0.file$clone_id == "clone1" | clone0.file$clone_id == "clone2", "Cell_barcodes"]
clone0b.barcodes <- clone0.file[clone0.file$Cell_barcodes %notin% clone0a.barcodes, "Cell_barcodes"]

# subset the clone file to subclone 1
clone1.file <- clone.file[clone.file$Cell_barcodes %in% clone1.barcodes, ]
clone1b.barcodes <- clone1.file[-grep("8q+", clone1.file$cnv_id), "Cell_barcodes"]
clone1a.barcodes <- clone1.file[grep("8q+", clone1.file$cnv_id), "Cell_barcodes"]
...
# change the identity of the subset of cells belonging to 0a to 0a
seurat.obj <- SetIdent(seurat.obj, cells = clone1b.barcodes, value = "1b")
seurat.obj <- SetIdent(seurat.obj, cells = clone1a.barcodes, value = "1a")
...
# manually add 0a
valid.clones <- factor(valid.clones, levels = c("0a", "0b", "1a", "1b", 0:10000))
valid.clones <- valid.clones[order(valid.clones)]

# remove clone 1
valid.clones <- valid.clones[-5]
...
for (subclone in valid.clones) {
  ...
  # run DGE analysis
  subclone.markers <- FindMarkers(seurat.obj, ident.1= subclone, ident.2="2", logfc.threshold = 0.25)
  
  # write all the markers to a file
  write.table(subclone.markers, paste0(o.dir, "/" , sample.tmp, "/DEG/markers_subclone", subclone,"_vs_2.txt") , quote=FALSE, sep="\t", col.names = T, row.names = T)
}
...
SCT.pseudobulk <- SCT.pseudobulk[order(SCT.pseudobulk$Sequencing20_C1_T2_T7_T12_ECB, decreasing = TRUE),, drop = FALSE]
...
for (i in 1:length(GSEA.results)){
  ...
  write.table(results, paste0(sample.tmp, "/DEG/", subclone.id[i], "_Hallmark_GSEA_vs_2.txt"),sep="\t", quote=FALSE, row.names = TRUE, col.names=TRUE)
}
...
all.wide.t$subclone2 <- 1

### KEEP AND ORDER GENE SETS YOU WANT TO PLOT

### GENE SET TO PLOT FOR SEQ 20
keeps <- c("HALLMARK_HYPOXIA","HALLMARK_TNFA_SIGNALING_VIA_NFKB", "HALLMARK_APOPTOSIS", "HALLMARK_MTORC1_SIGNALING",
           "HALLMARK_MYC_TARGETS_V1", "HALLMARK_ESTROGEN_RESPONSE_EARLY", "HALLMARK_ANGIOGENESIS", "HALLMARK_ALLOGRAFT_REJECTION",
           "HALLMARK_P53_PATHWAY", "HALLMARK_APICAL_JUNCTION")
...
allm <- allm[,c("subclone0a", "subclone0b", "subclone1a", "subclone1b",  "subclone2", "subclone3", "subclone4")]
...
rownames(marker.df)[nrow(marker.df)] <- "subclone2"
...
clone0.markers <- read.table(paste0(sample.tmp, "/DEG/markers_subclone1a_vs_2.txt"), stringsAsFactors = F, header = T)
...
## Estrogen
pdf(paste0(sample.tmp, "/DEG/GSEA_EnrichmentPlot_subclone1a_vs_2_EstrogenEarly.pdf"),width=5,height=4,pointsize=0.1)
viewGSEA(gsca3, gscName="MSig_H", gsName="HALLMARK_ESTROGEN_RESPONSE_EARLY", main.title = "Hypoxia")
dev.off()

## MYC
pdf(paste0(sample.tmp, "/DEG/GSEA_EnrichmentPlot_subclone1a_vs_2_MYC.pdf"),width=5,height=4,pointsize=0.1)
viewGSEA(gsca3, gscName="MSig_H", gsName="HALLMARK_MYC_TARGETS_V1", main.title = "Angiogenesis")
dev.off()
...
keeps <- c("chr3p","chr3q", "chr8p","chr8q")
