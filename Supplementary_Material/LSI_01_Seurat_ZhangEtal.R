...
## ZHANG ET AL
sample.list <- list.files(c.dir, full.names = T, pattern = "\\.rds", recursive = T, )

# get sample ids for ZHANG AND SATHE
sample.ids <- str_split_fixed(basename(sample.list), "_", 2)[,1]

## ZHANG ET AL - SEURAT OBJECTS
sample.list <- lapply(sample.list, read_rds)

# list to store seurat.objects in
seurat.obj.list <- list()

############################################################################
##               Read in the Seurat objects for ZHANG
############################################################################
...
for (i in 1:length(sample.list)){
  ...
  c.mtx <- sample.list[[i]]@assays$SCT@counts
...
seurat.obj.big <- merge(seurat.obj.list[[1]], y = seurat.obj.list[c(2:length(seurat.obj.list))],
                        add.cell.ids = sample.ids, project = "ZhangEtal2019_scRNA_gastric", merge.data = T)
...
#############################################################################################################################
# read in the seurat object from the step before
seurat.obj.big <- readRDS(file = paste0(o.dir, "/" , sample.tmp, "/", sample.tmp, "_seurat_obj.rds"))
...
sample.type[grep("CAG", sample.type)] <- "CAG"
sample.type[grep("NAG", sample.type)] <- "NAG"
sample.type[grep("EGC", sample.type)] <- "EGC"
sample.type[grep("IMS", sample.type)] <- "IMS"
sample.type[grep("IMW", sample.type)] <- "IMW"
...
new.cluster.ids <- c("Epithelial", "Epithelial", "Epithelial", "Epithelial", "Epithelial", "Epithelial", "Non-Epithelial", "Epithelial", "Epithelial", "Epithelial",
                     "Epithelial", "Epithelial", "Epithelial", "Non-Epithelial", "Epithelial", "Epithelial", "Epithelial", "Epithelial", "Non-Epithelial",
                     "Non-Epithelial", "Epithelial", "Non-Epithelial", "Epithelial", "Epithelial", "Non-Epithelial", "Epithelial", "Non-Epithelial", "Non-Epithelial", 
                     "Non-Epithelial", "Non-Epithelial", "Non-Epithelial", "Epithelial", "Epithelial", "Epithelial")
...
seurat.obj.big <- FindNeighbors(seurat.obj.big, dims = 1:75)
...
# # read seurat objects
# seurat.obj.big <- readRDS(paste0(o.dir, "/" , sample.tmp, "/", sample.tmp, "_Epithelial_seurat_obj.rds"))
...
new.cluster.ids <- c("Neck-like Cells", "GMCs", "PMCs", "PMCs", "MSCs", "Malignant Cells", "PMCs", "PMCs", "Endocrine Cells", "Enterocytes", "PMCs",
                     "MSCs", "PMCs", "Enterocytes", "Enteroendocrine", "PMCs", "PMCs", "Enterocytes", "MSCs", "Goblet Cells", "Neck-like Cells",
                     "Neck-like Cells", "Endocrine Cells", "PMCs", "Chief Cells", "Enteroendocrine Cells", "Unknown", "Endocrine Cells", "Parietal Cells", "Unknown")
...
# apply modulairty optimization techniques 
seurat.obj.big <- FindClusters(seurat.obj.big, resolution = 0.6)
...
# use canonical markers to match the unbiased clustering to known cell types for Zhang dataset
new.cluster.ids <- c("Neck-like Cells", "PMCs", "GMCs", "MSCs", "Malignant Cells", "PMCs", "PMCs", "PMCs", "Enteroendocrine Cells", "Enterocytes", "PMCs",
                     "Enteroendocrine Cells", "Enterocytes", "Unknown", "PMCs", "MSCs", "Goblet Cells", "PMCs", "MSCs", "MSCs", "Enteroendocrine Cells", "Chief Cells", "Parietal Cells")
...
# use canonical markers to match the unbiased clustering to known cell types for Zhang dataset
new.cluster.ids <- c("PMCs", "PMCs", "MSCs", "GMCs", "PMCs", "PMCs", "PMCs", "Malignant Cells","Endocrine Cells", "Enterocytes", "PMCs",
                     "Enteroendocrine Cells", "Enterocytes", "PMCs", "Malignant Cells", "MSCs", "Goblet Cells", "Neck-like Cells", "Neck-like Cells",
                     "MSCs", "Endocrine Cells", "Chief Cells", "Parietal Cells")
...
# ############################################################################
# ##        Compare PMCs/MSCs/GMCs from NAG to PMCs/MSCs/GMCs from CAG
# ############################################################################
# # read the seurat object back in your environment
# epithelial.seurat.obj <- readRDS(paste0(sample.tmp, "_epithelial_seurat_obj.rds"))
# epithelial.seurat.obj <- readRDS("/labs/ccurtis2/mjprzy/scRNA_analysis/seurat/merged_data_ZhangEtal/epithelial_subset/merged_data_ZhangEtal_epithelial_seurat_obj.rds")
# 
# # check metadata
# head(epithelial.seurat.obj@meta.data)
# 
# # check cell identities
# levels(epithelial.seurat.obj)
# 
# # make a tsne plot with splitting to each sample
# pdf(paste("plots/", sample.tmp, "_Cells_by_Sample.pdf", sep = ""), width = 40, height = 20)
# DimPlot(epithelial.seurat.obj, reduction = "tsne", group.by = "sample_type", split.by = "orig.ident")
# dev.off()
# 
# # make a column with the Cell type and information which sample it is
# epithelial.seurat.obj$SampleCellType <- paste(epithelial.seurat.obj$CellType, epithelial.seurat.obj$sample_type, sep = "_")
# Idents(epithelial.seurat.obj) <- "SampleCellType"
# 
# # find all markers 
# mucosal.markers <- FindMarkers(epithelial.seurat.obj, ident.1 = c("PMCs_CAG","GMCs_CAG","MSCs_CAG"), ident.2 = c("PMCs_NAG","GMCs_NAG","MSCs_NAG"),
#                                min.pct = 0.25, test.use = "DESeq2")
# # mucosal.markers <- mucosal.markers[mucosal.markers$avg_logFC > 0.5 | mucosal.markers$avg_logFC < -0.5,]
# head(mucosal.markers, n = 5)
# 
# # write.table(mucosal.markers, paste0(sample.tmp, "_mucosalMarkers_NAG_CAG.txt"), sep = "\t", quote = F, col.names = T)
# write.table(mucosal.markers, "/labs/ccurtis2/mjprzy/scRNA_analysis/seurat/merged_data_ZhangEtal/epithelial_subset/merged_data_ZhangEtal_mucosalMarkers_NAG_CAG.txt", sep = "\t", quote = F, col.names = T)

############################################################################
##        Compare PMCs/MSCs/GMCs from NAG to PMCs/MSCs/GMCs from IM
############################################################################

# # find all markers 
# IM.markers <- FindMarkers(epithelial.seurat.obj, ident.1 = c("PMCs_IMS","GMCs_IMS","MSCs_IMS", "PMCs_IMW","GMCs_IMW","MSCs_IMW"), ident.2 = c("PMCs_NAG","GMCs_NAG","MSCs_NAG"),
#                           min.pct = 0.25, test.use = "DESeq2")
# # IM.markers <- IM.markers[IM.markers$avg_logFC > log(2) | IM.markers$avg_logFC < -log(2),]
# head(IM.markers)
# 
# # write.table(IM.markers, paste0(sample.tmp, "_mucosalMarkers_NAG_IM.txt"), sep = "\t", quote = F, col.names = T)
# write.table(IM.markers, "/labs/ccurtis2/mjprzy/scRNA_analysis/seurat/merged_data_ZhangEtal/epithelial_subset/merged_data_ZhangEtal_mucosalMarkers_NAG_IM.txt", sep = "\t", quote = F, col.names = T)
