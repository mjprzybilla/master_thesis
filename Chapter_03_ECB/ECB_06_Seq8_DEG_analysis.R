#############################################################################################################################
##                                                                                                                      
##  Analysis of individual subclones in scRNA-seq with ECB information from Sequencing 8
##                                                                                                                      
##  Date: 23 July 2020                                                                                                                    
##  
...
getAveAltUp <- function(rg,propup){
  ...
  degs <- read.table(paste0(sample.tmp, "/DEG/markers_",rg,"_vs_1.txt"), sep="\t", header = T, stringsAsFactors = T)
...
plotDEGs <- function(rg){
  ...
  pdf(paste0(o.dir, "/", sample.tmp, "/DEG/markers_",rg,"_vs_1.pdf"),width=9.5,height=1.5,pointsize=0.1)
...
Idents(seurat.obj) <- "SCT_snn_res.0.4"
...
cnv.clone.files <- cnv.clone.files[grep("Sequencing8", cnv.clone.files)]
...
clone0a.barcodes <- clone0.file[grep("4q-", clone0.file$cnv_id), "Cell_barcodes"]
clone0b.barcodes <- clone0.file[-grep("4q-", clone0.file$cnv_id), "Cell_barcodes"]
...
ECB.clones <- table(Idents(seurat.obj)) > 10
valid.clones <- names(ECB.clones[ECB.clones == T])

# manually add 0a
valid.clones <- factor(valid.clones, levels = c("0a", "0b", 1:10000))
valid.clones <- valid.clones[order(valid.clones)]

# remove clone 1
valid.clones <- valid.clones[-3]
...
for (subclone in valid.clones) {
  ...
  # run DGE analysis
  subclone.markers <- FindMarkers(seurat.obj, ident.1= subclone, ident.2="1", logfc.threshold = 0.25)
  
  # write all the markers to a file
  write.table(subclone.markers, paste0(o.dir, "/" , sample.tmp, "/DEG/markers_subclone", subclone,"_vs_1.txt") , quote=FALSE, sep="\t", col.names = T, row.names = T)
}
...
SCT.pseudobulk <- SCT.pseudobulk[order(SCT.pseudobulk$Sequencing8_C5T2R2_cr3, decreasing = TRUE),, drop = FALSE]
...
for (i in 1:length(GSEA.results)){
  ...
  write.table(results, paste0(sample.tmp, "/DEG/", subclone.id[i], "_Hallmark_GSEA_vs_1.txt"),sep="\t", quote=FALSE, row.names = TRUE, col.names=TRUE)
}
...
all.wide.t$subclone1 <- 1

### KEEP AND ORDER GENE SETS YOU WANT TO PLOT

### GENE SET TO PLOT FOR SEQ 8
keeps <- c("HALLMARK_HYPOXIA","HALLMARK_TNFA_SIGNALING_VIA_NFKB","HALLMARK_INTERFERON_GAMMA_RESPONSE",
           "HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION","HALLMARK_CHOLESTEROL_HOMEOSTASIS","HALLMARK_APOPTOSIS",
           "HALLMARK_COMPLEMENT","HALLMARK_GLYCOLYSIS","HALLMARK_INFLAMMATORY_RESPONSE",
...
allm <- allm[,c("subclone0a", "subclone0b", "subclone1", "subclone39", "subclone13", "subclone56", "subclone11", "subclone6", "subclone9")]
...
rownames(marker.df)[nrow(marker.df)] <- "subclone1"
...
clone0.markers <- read.table(paste0(sample.tmp, "/DEG/markers_subclone0a_vs_1.txt"), stringsAsFactors = F, header = T)
...
gsca2 <- analyze(gsca1, para=list(pValueCutoff=0.05, pAdjustMethod="BH", nPermutations=10000, minGeneSetSize=1, exponent=1), doGSOA = FALSE)
...
## EMT
pdf(paste0(sample.tmp, "/DEG/GSEA_EnrichmentPlot_subclone0a_vs_1_EMT.pdf"),width=5,height=4,pointsize=0.1)
viewGSEA(gsca3, gscName="MSig_H", gsName="HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION", main.title = "Epithelial Mesenchymal Transition")
dev.off()

## G2M
pdf(paste0(sample.tmp, "/DEG/GSEA_EnrichmentPlot_subclone0a_vs_1_G2M.pdf"),width=5,height=4,pointsize=0.1)
viewGSEA(gsca3, gscName="MSig_H", gsName="HALLMARK_G2M_CHECKPOINT", main.title = "G2M Checkpoint")
dev.off()

## TNFA
pdf(paste0(sample.tmp, "/DEG/GSEA_EnrichmentPlot_subclone0a_vs_1_TNFA.pdf"),width=5,height=4,pointsize=0.1)
viewGSEA(gsca3, gscName="MSig_H", gsName="HALLMARK_TNFA_SIGNALING_VIA_NFKB", main.title = "TNFa signaling via NFKß")
dev.off()

## MYC
pdf(paste0(sample.tmp, "/DEG/GSEA_EnrichmentPlot_subclone0a_vs_1_MYC.pdf"),width=5,height=4,pointsize=0.1)
viewGSEA(gsca3, gscName="MSig_H", gsName="HALLMARK_MYC_TARGETS_V1", main.title = "MYC targets")
dev.off()
...
keeps <- c("chr3p","chr3q", "chr4p","chr4q","chr9p","chr9q", "chr20p","chr20q")
