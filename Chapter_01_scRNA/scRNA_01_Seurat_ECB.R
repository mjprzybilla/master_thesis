#############################################################################################################################
##                                                                                                                      
##  Analysis of single-cell RNA-seq data with ECB information using Seurat
##                                                                                                                      
##  Date: 23 July 2020                                                                                                                    
##  
...
c.dir <- "/labs/ccurtis2/mjprzy/infercnv_gastric/Sequencing8_C5T2R2_cr3/outs/"
# c.dir <- "/labs/ccurtis2/mjprzy/infercnv_gastric/Sequencing18_D3_APA4_Early/outs/"
# c.dir <- "/labs/ccurtis2/mjprzy/infercnv_gastric/Sequencing20_C1_T2_T7_T12_ECB/Markdown/outs/"
# c.dir <- "/labs/ccurtis2/mjprzy/infercnv_gastric/Sequencing25_6077_C5_T2_T12/Markdown/outs/"
...
## SEQ8
# cell.metadata <- read_delim("/labs/ccurtis2/mjprzy/hashECB_data_updated/Sequencing8_C5T2R2/ECB_data/ecb_outs/combined_outs/Cell_Metadata_full.txt", delim = "\t", col_names = T)
## SEQ18  
# cell.metadata <- read_delim("/labs/ccurtis2/mjprzy/hashECB_data_updated/Sequencing18_D3_APA4_Early/ECB_data/ecb_outs/combined_outs/Cell_Metadata_full.txt", delim = "\t", col_names = T)
## SEQ20
# cell.metadata <- read_delim("/labs/ccurtis2/mjprzy/hashECB_data_updated/Sequencing20_C1_T2_T7_T12_ECB/ECB_data/ecb_outs/combined_outs/Cell_Metadata_full.txt", delim = "\t", col_names = T)
## SEQ25
cell.metadata <- read_delim("/labs/ccurtis2/mjprzy/hashECB_data_updated/Sequencing25_6077_C5_T2_T12/ECB_data/ecb_outs/combined_outs/Cell_Metadata_full.txt", delim = "\t", col_names = T)
...
# ECB
seurat.obj <- subset(seurat.obj, subset = HashTag != "Multiplet")
seurat.obj <- subset(seurat.obj, subset = ECB_RG != "Na")

# # # for Seq18
# seurat.obj <- subset(seurat.obj, subset = HashTag != "6077_P_D3_R3_T4")
# seurat.obj <- subset(seurat.obj, subset = HashTag != "6077_P_D3_R3_T8")
# seurat.obj <- subset(seurat.obj, subset = HashTag != "6077_P_D3_R3_T1")
...
# find NNs
seurat.obj<- FindNeighbors(seurat.obj, dims = 1:40, assay = "RTAM", reduction = "pca_RTAM")

# find clusters
seurat.obj <- FindClusters(seurat.obj, resolution = 0.6)

# run umap
seurat.obj <- RunUMAP(seurat.obj, dims = 1:40, assay = "RTAM", reduction = "pca_RTAM", reduction.name = "umap_RTAM", reduction.key = "umapRTAM_")
...
# run tsne
seurat.obj <- RunTSNE(seurat.obj, dims = 1:40, assay = "RTAM", reduction = "pca_RTAM", reduction.name = "tsne_RTAM", reduction.key = "tsneRTAM_")
...
Idents(seurat.obj) <- "ECB_RG" # EL single
...
# ###########################################################################
# #             MAKE UMAP EMBEDDING WITH ECB RG 0A AND OB
# ###########################################################################
# # read in the copy number clone file for C5 R2 T2
# clone.id.file <- read.table("/labs/ccurtis2/mjprzy/infercnv_gastric/freeze/arm_level/Sequencing8_C5T2R2_cr3_WT_cnv_clones.txt", header = T)
# 
# # get the ECB 0A
# ECB.zeroA <- clone.id.file[clone.id.file$clone_id == "clone8", "Cell_barcodes"]
# ECB.zeroA <- gsub("\\.", "-", ECB.zeroA)
# 
# # get the barcodes for ECB 0
# ECB.zeroB <- seurat.obj@meta.data[seurat.obj@meta.data$ECB_RG == 0, "Cell_Barcode"]
# ECB.zeroB <- ECB.zeroB[ECB.zeroB %notin% ECB.zeroA]
# 
# DimPlot(seurat.obj, cells.highlight= c(ECB.zeroA, ECB.zeroB), cols.highlight = c("red", "black"), cols= "grey", pt.size = 1)
# 
# # make individual plots per HashTag
# plotlist <- list()
# for (i in 1:length(sample.idents)){
# 
#   # which ident?
#   sample.ident <- sample.idents[i]
# 
#   # get list of barcodes with this ident
#   cells.ident <- seurat.obj.big@meta.data[seurat.obj.big$orig.ident == sample.ident,]
#   cells.ident <- cells.ident$Cell_Barcode
# 
#   # make plots with colouring according to the sample.ident
#   plot <- DimPlot(seurat.obj.big, cells.highlight= list(cells.ident), cols.highlight = c("darkblue"), cols= "grey", pt.size = 1) + labs(title = sample.ident)
# 
#   # save in list
#   plotlist[[i]] <- plot + NoLegend()
# 
# }
# # plot the list
# nCol <- floor(sqrt(length(plotlist)))
# pdf(paste("plots/", sample.tmp, "_SampleIdentUMAPplot", ".pdf", sep = ""), width = 40, height = 25)
...
#                               "GMCs" = "lightgrey", "Endocrine Cells" = "lightgrey", "PCs" = "lightgrey", "Non-Mucosal-like malignant Cells" = magma(9)[3],
